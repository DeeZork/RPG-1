...
class Arms extends Thing {
    ...
    public static int[] gen(int stage) {
        ...
        gen[1] = random.nextInt(8);// имя,всего 8 видов оружия/защиты
        gen[2] = random.nextInt(stage) + 1;// уровень
        if (gen[0] == 0) {
            gen[3] = random.nextInt(stage * gen[1] * 10) + 1;// сила атаки
            gen[4] = random.nextInt(stage * gen[1] * 3) + 1;// защита
        } else {
            gen[3] = random.nextInt(stage * gen[1] * 3) + 1;// сила атаки
            gen[4] = random.nextInt(stage * gen[1] * 10) + 1;// защита
        }
        gen[5] = random.nextInt((int) (stage * gen[1] * (gen[0] + 1) * 1.5)) + 1;//вес
        gen[6] = random.nextInt(stage * (gen[3] + gen[4] - gen[5] + 1)) + 1;// цена
        return gen;
    }
